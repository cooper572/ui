...
function PosterCard({ movie }: { movie: RailMovie }) {
  return (
    <Card
      size="sm"
      className={cn(
        "group shrink-0 gap-0 overflow-hidden rounded-2xl border border-white/10 bg-black/95 text-white transition-all duration-300 hover:-translate-y-1 hover:border-red-500/40 hover:shadow-2xl hover:shadow-red-900/20 w-[185px]"
      )}
    >
      <div className="relative h-[278px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-end p-3 opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0 translate-y-2">
      ...
      <CardContent className="space-y-1.5 px-2.5 py-2.5">
        <p className="truncate text-sm font-semibold leading-tight text-zinc-100">{movie.title}</p>
